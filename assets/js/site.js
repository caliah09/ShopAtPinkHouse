...
(function () {
  ...
  var Bag = {
    ...
    /* ---- THE SEAM -------------------------------------------------------
       ...
       Same-tab, not a new one: Shopify's checkout page refuses to load in an
       iframe (PCI/fraud-prevention, same as every real payment processor), so
       the closest this site can get to feeling like "checkout stays on the
       site" is a same-tab handoff instead of a popup. Brand the destination
       itself under Shopify admin -> Settings -> Checkout -> Customize so it
       carries the site's colours and logo rather than looking like a jump to
       a different store. */
    checkout: function () {
      ...
      if (allLinked) {
        ...
        location.assign('https://' + domain + '/cart/' + parts.join(','));
        return;
      }
      var url = 'https://' + domain;
      if (!this.lines.length) { location.assign(url); return; }
      // give the toast a moment on screen before the page unloads under it
      toast('Some items aren’t in the store yet — opening the shop');
      setTimeout(function () { location.assign(url); }, 900);
    },
